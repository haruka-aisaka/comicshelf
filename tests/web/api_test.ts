...
Deno.test("API E2E: 全体フロー (rebuild → list → page取得 → 既読更新)", async () => {
  ...
  try {
    ...
    // 2. 一覧取得 (明示的にタイトルソート指定 — 既定は modified に変更されたため)
    const listRes = await w.app.app.request("/api/books?sort=title");
    ...
    // 2b. 既定ソートが modified になっていることを確認
    const defRes = await w.app.app.request("/api/books");
    const def = await defRes.json();
    assertEquals(def.sort, "modified");
