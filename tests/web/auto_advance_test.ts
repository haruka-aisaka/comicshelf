...
Deno.test("auto-advance: 不正な storedSec は MIN_INTERVAL_SEC に丸める", () => {
  ...
  const auto = createAutoAdvance(h.deps);
  assertEquals(auto.inspect().intervalSec, 1.5);
});
...
Deno.test("auto-advance: 連続 tick でページが進み続ける", () => {
  ...
  const auto = createAutoAdvance(h.deps);
  auto.setIntervalSec(1.5); // 現状の MIN。 5 秒で 3 回進む
  auto.toggleUserStop();
  h.advance(5000);
  assertEquals(h.getMoveForwardCalls(), 3);
  assertEquals(h.getCurrentPage(), 3);
});
...
Deno.test("auto-advance: setIntervalSec で localStorage に保存される", () => {
  ...
  // クランプ動作 (min 未満は MIN=1.5、 max 超は MAX=12)
  auto.setIntervalSec(0.3);
  assertEquals(h.getStorage().k, "1.5");
  auto.setIntervalSec(30);
  assertEquals(h.getStorage().k, "12");
});
