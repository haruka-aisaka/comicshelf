...
Deno.test("migration: v2 DB を開くと root_id が defaultRootId で埋まる", async () => {
  ...
  try {
    ...
    try {
      ...
      // schema_meta が最新版 (v4) に更新されている
      const ver = db
        .prepare("SELECT value FROM schema_meta WHERE key = 'version'")
        .get<{ value: string }>();
      assertEquals(ver?.value, "4");
...
Deno.test("migration: 新規 DB は v4 で立ち上がる + favorites テーブルがある", () => {
...
Deno.test("migration: v2 → v4 でも read_states / favorites の整合性が保たれる", async () => {
